class WebpayInit
   ...
   def self.nullify_transaction(token, base_url, authorizationCode, authorizedAmount, buyOrder, nullifyAmount)
            self.init_webpay(base_url)
            return @webpay.getNullifyTransaction.nullify(authorizationCode, authorizedAmount, buyOrder, nullifyAmount, ENV['webpay_commerce_code'].to_s)
   end           
   ...
   File.open(directory, 'a+') do |f|
      if type.to_s == "request" && action.to_s == "init_transaction"
         ...
         f << "\t [returnURL] => https://frutaconsentido.com" + ENV['webpay_return_url'].to_s + "\n"
         f << "\t [finalURL] https://frutaconsentido.com" + ENV['webpay_final_url'].to_s + "\n"
         f << "\t [transactionDetails]" + "\n"
         f << "\t (" + "\n"
         f << "\t\t [amount] => " + response_document.xpath("//amount").text.to_s + "\n"
         f << "\t\t [commerceCode] => " + ENV['webpay_commerce_code'].to_s + "\n"
         f << "\t\t [buyOrder] =>" + id.to_s.split(':')[1].to_s + "\n"
         f << "\t )" + "\n"
   ...
   def self.init_webpay(base_url)
         ...
         config.commerce_code = ENV['webpay_commerce_code']
         config.environment = ENV['environment']
         config.private_key = OpenSSL::PKey::RSA.new(File.read(ENV['webpay_client_private_key']))
         config.public_cert = OpenSSL::X509::Certificate.new(File.read(ENV['webpay_client_certificate']))
         config.webpay_cert = OpenSSL::X509::Certificate.new(File.read(ENV['webpay_tbk_certificate']))        
         puts 'commerce code: ' + ENV['webpay_commerce_code'].to_s
         puts 'env: ' + ENV['environment'].to_s
         puts 'private_key: ' + OpenSSL::PKey::RSA.new(File.read(ENV['webpay_client_private_key'])).to_s
         puts 'public_cert: ' + OpenSSL::X509::Certificate.new(File.read(ENV['webpay_client_certificate'])).to_s
         puts 'webpay_cert: ' + OpenSSL::X509::Certificate.new(File.read(ENV['webpay_tbk_certificate'])).to_s
         @webpay = libwebpay.getWebpay(config)
         @urlReturn = base_url + ENV['webpay_return_url'].to_s
         @urlFinal = base_url + ENV['webpay_final_url'].to_s
   end               
